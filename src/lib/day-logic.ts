...
export function getParentForState(
  state: DayState,
  dateStr: string = getDateString(),
): Parent {
...
export async function switchDay(): Promise<{
  success: boolean
  newParent: Parent
  newState: DayState
}> {
  ...
  return {
    success: true,
    newParent: getParentForState(state),
    newState: state,
  }
}
...
export function getDebtInfoForState(state: DayState): {
  owedTo: Parent | null
  amount: number
} {
